...
@modulus.sym.main(config_path="conf", config_name="DeepCA")
def run(cfg: ModulusConfig) -> None:
    ...
    ns = NavierStokes(nu = nu, rho = nd.ndim(rho), dim = 3, time = False)
    ...
    + [invert_net.make_node(name = "invert_net")]
    + Scaler(["u","v","w","p"],
    ...
    for i,plane in enumerate(all_planes):
        ...
        integral_continuity = IntegralBoundaryConstraint(
            ...
            lambda_weighting= {"normal_dot_vel": 1000},
        )
    ...
    input_bend, output_bend, nr_points = get_data(
        df_path= os.path.join(data_path, f"U0{key}_NotUni_0.5K_Train.csv"),
    ...
    flow_data = np.full((nr_points, 1), fill_value=1000.0)
    ...
    val_df = os.path.join(data_path, f"U0{key}_NotUni_0.5K_Validation.csv")
    ...
    csv_var = csv_to_dict(
        to_absolute_path(os.path.join(data_path, f"U0{key}_NotUni_0.5K_Validation.csv")), mapping
    )
...
if __name__ == "__main__":
    ...
    # sqrt( (pred_u-true_u)^2 + (pred_v -true_v)^2 +(pred_w-true_w)^2 )
    # pred_u*ihat + pred_u*jhat + pred_w*khat
...
# /zhome/e3/5/167986/Desktop/PINN_Bachelor/Anton_leg/Navier Stokes 3d/PipeInverse.py