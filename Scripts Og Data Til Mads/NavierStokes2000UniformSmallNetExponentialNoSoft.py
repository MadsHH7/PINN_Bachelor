...
@modulus.sym.main(config_path="conf", config_name="configSmallNetExponentialNoSoft")
def run(cfg: ModulusConfig) -> None:
    ...
    for i,plane in enumerate(all_planes):
        ...
        integral_continuity = IntegralBoundaryConstraint(
            ...
            lambda_weighting= {"normal_dot_vel": 1000}, # Testing very high lambda wieghting for integral condition.
        )
        Pipe_domain.add_constraint(integral_continuity, f"integral_plane_{i}")

    data_path = f"/zhome/e3/5/167986/Desktop/PINN_Bachelor/batch_output/SparseDataNormalProblem"
    key = "pt1"

    input_bend, output_bend, nr_points = get_data(
        df_path= os.path.join(data_path, f"U0{key}_2000_Uniform_Train.csv"),
    ...
    val_df = os.path.join(data_path, f"U0{key}_2000_Uniform_Validation.csv")
